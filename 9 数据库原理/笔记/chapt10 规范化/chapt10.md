...
## 闭卷客观题复习提示
- **各范式定义**：1NF是属性不可再分；2NF消除部分依赖；3NF消除传递依赖；BCNF决定因素必须是候选码。`【闭卷重点·必背】`
- **函数依赖判断**：平凡依赖与非平凡依赖，完全依赖与部分依赖的定义。`【闭卷重点·常考】`
- **数据异常种类**：插入异常、删除异常、修改异常的定义与辨析。`【闭卷重点·常考】`
- **范式强弱关系**：BCNF => 3NF => 2NF => 1NF。`【闭卷重点·必背】`
...
```text
一个关系属于 1NF，当且仅当每个属性在每个元组中都只含有一个原子值。 `【闭卷重点·必背】`
```
...
```text
2NF = 1NF + 消除非主属性对主键的部分函数依赖 `【闭卷重点·常考】`
```
...
```text
3NF = 2NF + 消除非主属性对主键的传递函数依赖 `【闭卷重点·常考】`
```
...
```text
若关系 R 中每一个非平凡函数依赖 X -> Y 的决定因素 X 都是 R 的候选键，则 R 属于 BCNF。 `【闭卷重点·必背】`
```
