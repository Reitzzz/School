...
> * **深入巩固了“3 第三阶段：核心拼装（DML - 查询）”**：大量使用了 `GROUP BY` 与聚合函数进行各类统计计算。
> * **重点覆盖了“4 第四阶段：高级扩展拼装”**：不仅包含 **4.1 多表连接 (JOIN)** 和 **4.2 子查询**，还核心考察了 **4.3 EXISTS 与除法查询**（尤其是使用双重 `NOT EXISTS` 表达“所有”语义的高级用法）。
> * *(未涉及：DDL、数据更新、集合运算及视图权限等内容。)*
