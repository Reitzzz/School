...
有一个员工项目数据库，包含部门 DEPT、员工 EMP、项目 PROJ、员工参与项目情况 EP 四张表。一个员工属于一个部门，一个部门有多个员工；一个员工可同时参加多个项目，一个项目也可由多个员工共同开发。用 SQL 的 DDL 语言建立这四张表、表的主码、外码和索引。其中：在部门表 DEPT 上按部门名称建立升序唯一索引；在员工表 EMP 上按部门建立索引，按职工工资降序建立索引；在员工项目表 EP 上按员工号升序、项目号降序建立唯一索引。
